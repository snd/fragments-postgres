...
optionsToConfig = function(options) {
  ...
  config.nameToInsertableColumns = options.nameToInsertableColumns || function(name) {
    return name + 'InsertableColumns';
  };
  config.nameToUpdateableColumns = options.nameToUpdateableColumns || function(name) {
...
configure = function(options) {
  ...
  sources.insert = function(name) {
    var factory, parsed;
    parsed = sources.parseInsert(name);
    if (parsed == null) {
      return;
    }
    ...
    factory.__inject = [config.nameToTable(parsed.name), config.nameToInsertableColumns(parsed.name)];
  ...
  sources.update = function(name) {
    var factory, parsed;
    parsed = sources.parseUpdate(name);
    if (parsed == null) {
      return;
    }
    factory = function(table, allowedColumns) {
    ...
    factory.__inject = [config.nameToTable(parsed.name), config.nameToUpdateableColumns(parsed.name)];
