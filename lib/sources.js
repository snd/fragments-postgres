// Generated by CoffeeScript 1.9.3
var configure, helfer, optionsToConfig,
  slice = [].slice;

helfer = require('helfer');

optionsToConfig = function(options) {
  var config;
  config = {};
  config.keyPrefix = 'fragments_';
  config.tableWordArray = helfer.splitCamelcase(options.tableWords || 'table');
  config.tableToKey = options.tableToKey || function(tableName) {
    var words;
    words = helfer.splitUnderscore(tableName).concat(config.tableWordArray);
    return helfer.joinCamelcase(words);
  };
  config.tableToInsertableColumns = options.tableToInsertableColumns || function(table) {
    return helfer.snakeToCamel(table) + 'InsertableColumns';
  };
  config.tableToUpdateableColumns = options.tableToUpdateableColumns || function(table) {
    return helfer.snakeToCamel(table) + 'UpdateableColumns';
  };
  config.stripPrefix = function(string) {
    if (0 !== string.indexOf(config.keyPrefix)) {
      return;
    }
    return string.slice(config.keyPrefix.length);
  };
  return config;
};

configure = function(options) {
  var config, sources;
  if (options == null) {
    options = {};
  }
  config = optionsToConfig(options);
  sources = {};
  sources.table = function(key) {
    var containsTable, doesntBeginWithTable, endsWithTable, factory, ref, ref1, results, tableName, words;
    key = config.stripPrefix(key);
    if (key == null) {
      return;
    }
    words = helfer.splitCamelcase(key);
    results = helfer.splitArrayWhereSequence(words, config.tableWordArray);
    containsTable = results.length === 2;
    if (!containsTable) {
      return;
    }
    doesntBeginWithTable = ((ref = results[0]) != null ? ref.length : void 0) !== 0;
    if (!doesntBeginWithTable) {
      return;
    }
    endsWithTable = ((ref1 = results[1]) != null ? ref1.length : void 0) === 0;
    if (!endsWithTable) {
      return;
    }
    tableName = helfer.joinUnderscore(results[0]);
    factory = function(fragments_mesa) {
      return fragments_mesa.table(tableName);
    };
    factory.__inject = ['fragments_mesa'];
    factory.__source = 'generated by fragments-postgres.table';
    return factory;
  };
  sources.parseSelect = function(key) {
    var order, orderProcessed, ref, ref1, ref2, table, tableWhere, tableWhereOrder, tableWhereOrderConnection, type, where, whereProcessed, withConnection, words;
    key = config.stripPrefix(key);
    if (key == null) {
      return;
    }
    words = helfer.splitCamelcase(key);
    type = words[0];
    if (type !== 'first' && type !== 'select') {
      return;
    }
    tableWhereOrderConnection = words.slice(1);
    ref = helfer.splitArrayWhereSequence(tableWhereOrderConnection, ['with', 'connection']), tableWhereOrder = ref[0], withConnection = ref[1];
    ref1 = helfer.splitArrayWhereSequence(tableWhereOrder, ['order', 'by']), tableWhere = ref1[0], order = 2 <= ref1.length ? slice.call(ref1, 1) : [];
    ref2 = helfer.splitArrayWhereSequence(tableWhere, 'where'), table = ref2[0], where = 2 <= ref2.length ? slice.call(ref2, 1) : [];
    if (table.length === 0) {
      return;
    }
    whereProcessed = where.filter(function(x) {
      return x.length !== 0;
    }).map(helfer.joinUnderscore);
    orderProcessed = order.filter(function(x) {
      return x.length !== 0;
    }).map(function(x) {
      var column, direction, last;
      last = x[x.length - 1];
      if (last === 'asc' || last === 'desc') {
        column = x.slice(0, -1);
        direction = last.toUpperCase();
      } else {
        column = x;
        direction = 'ASC';
      }
      return {
        column: helfer.joinUnderscore(column),
        direction: direction
      };
    });
    return {
      type: type,
      table: helfer.joinUnderscore(table),
      order: orderProcessed,
      where: whereProcessed,
      withConnection: withConnection != null
    };
  };
  sources.selectFirst = function(key) {
    var factory, parsed;
    parsed = sources.parseSelect(key);
    if (!((parsed != null) && parsed.type === 'first')) {
      return;
    }
    factory = function(table) {
      return function() {
        var args, connection, index, order, query;
        args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
        query = table;
        index = 0;
        parsed.where.forEach(function(x) {
          var condition;
          condition = {};
          condition[x] = args[index++];
          return query = query.where(condition);
        });
        if (parsed.order.length !== 0) {
          order = parsed.order.map(function(x) {
            return x.column + ' ' + x.direction;
          }).join(', ');
          query = query.order(order);
        }
        if (parsed.withConnection) {
          connection = args[index];
          if (connection == null) {
            throw new Error(key + " must be called with connection as argument number " + (index + 1));
          }
          query = query.setConnection(connection);
        }
        return query.first();
      };
    };
    factory.__inject = [config.tableToKey(parsed.table)];
    factory.__source = 'generated by fragments-postgres.selectFirst';
    factory.__parsed = parsed;
    return factory;
  };
  sources.select = function(key) {
    var factory, parsed;
    parsed = sources.parseSelect(key);
    if (!((parsed != null) && parsed.type === 'select')) {
      return;
    }
    factory = function(table) {
      return function() {
        var args, connection, index, order, q;
        args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
        q = table;
        index = 0;
        parsed.where.forEach(function(x) {
          var condition;
          condition = {};
          condition[x] = args[index++];
          return q = q.where(condition);
        });
        if (parsed.order.length !== 0) {
          order = parsed.order.map(function(x) {
            return x.column + ' ' + x.direction;
          }).join(', ');
          q = q.order(order);
        }
        if (parsed.withConnection) {
          connection = args[index];
          if (connection == null) {
            throw new Error(key + " must be called with connection as argument number " + (index + 1));
          }
          q = q.setConnection(connection);
        }
        return q.find();
      };
    };
    factory.__inject = [config.tableToKey(parsed.table)];
    factory.__source = 'generated by fragments-postgres.select';
    factory.__parsed = parsed;
    return factory;
  };
  sources.parseInsert = function(key) {
    var ref, table, withConnection, words;
    key = config.stripPrefix(key);
    if (key == null) {
      return;
    }
    words = helfer.splitCamelcase(key);
    if ('insert' !== words[0]) {
      return;
    }
    ref = helfer.splitArrayWhereSequence(words.slice(1), ['with', 'connection']), table = ref[0], withConnection = ref[1];
    if (table.length === 0) {
      return;
    }
    return {
      table: helfer.joinUnderscore(table),
      withConnection: withConnection != null
    };
  };
  sources.insert = function(key) {
    var factory, parsed;
    parsed = sources.parseInsert(key);
    if (parsed == null) {
      return;
    }
    factory = function(table, allowedColumns) {
      return function() {
        var args, connection, data, index, query;
        data = arguments[0], args = 2 <= arguments.length ? slice.call(arguments, 1) : [];
        query = table;
        index = 0;
        if (parsed.withConnection) {
          connection = args[index];
          if (connection == null) {
            throw new Error(key + " must be called with connection as argument number " + (index + 1));
          }
          query = query.setConnection(connection);
        }
        return query.allow(allowedColumns).insert(data);
      };
    };
    factory.__inject = [config.tableToKey(parsed.table), config.tableToInsertableColumns(parsed.table)];
    factory.__source = 'generated by fragments-postgres.insert';
    factory.__parsed = parsed;
    return factory;
  };
  sources.parseUpdate = function(key) {
    var ref, ref1, table, tableWhere, where, withConnection, words;
    key = config.stripPrefix(key);
    if (key == null) {
      return;
    }
    words = helfer.splitCamelcase(key);
    if ('update' !== words[0]) {
      return;
    }
    ref = helfer.splitArrayWhereSequence(words.slice(1), ['with', 'connection']), tableWhere = ref[0], withConnection = ref[1];
    ref1 = helfer.splitArrayWhereSequence(tableWhere, 'where'), table = ref1[0], where = 2 <= ref1.length ? slice.call(ref1, 1) : [];
    if (table.length === 0) {
      return;
    }
    if (where.length === 0) {
      return;
    }
    return {
      table: helfer.joinUnderscore(table),
      where: where.map(function(x) {
        return helfer.joinUnderscore(x);
      }),
      withConnection: withConnection != null
    };
  };
  sources.update = function(key) {
    var factory, parsed;
    parsed = sources.parseUpdate(key);
    if (parsed == null) {
      return;
    }
    factory = function(table, allowedColumns) {
      return function() {
        var args, connection, data, index, query;
        data = arguments[0], args = 2 <= arguments.length ? slice.call(arguments, 1) : [];
        query = table;
        index = 0;
        parsed.where.forEach(function(x) {
          var condition;
          condition = {};
          condition[x] = args[index++];
          return query = query.where(condition);
        });
        if (parsed.withConnection) {
          connection = args[index];
          if (connection == null) {
            throw new Error(name + " must be called with connection as argument number " + (index + 1));
          }
          query = query.setConnection(connection);
        }
        return query.allow(allowedColumns).update(data);
      };
    };
    factory.__inject = [config.tableToKey(parsed.table), config.tableToUpdateableColumns(parsed.table)];
    factory.__source = 'generated by fragments-postgres.update';
    factory.__parsed = parsed;
    return factory;
  };
  sources.parseDelete = function(key) {
    var ref, ref1, table, tableWhere, where, withConnection, words;
    key = config.stripPrefix(key);
    if (key == null) {
      return;
    }
    words = helfer.splitCamelcase(key);
    if ('delete' !== words[0]) {
      return;
    }
    ref = helfer.splitArrayWhereSequence(words.slice(1), ['with', 'connection']), tableWhere = ref[0], withConnection = ref[1];
    ref1 = helfer.splitArrayWhereSequence(tableWhere, 'where'), table = ref1[0], where = 2 <= ref1.length ? slice.call(ref1, 1) : [];
    if (table.length === 0) {
      return;
    }
    if (where.length === 0) {
      return;
    }
    return {
      table: helfer.joinUnderscore(table),
      where: where.map(function(x) {
        return helfer.joinUnderscore(x);
      }),
      withConnection: withConnection != null
    };
  };
  sources["delete"] = function(key) {
    var factory, parsed;
    parsed = sources.parseDelete(key);
    if (parsed == null) {
      return;
    }
    factory = function(table) {
      return function() {
        var args, connection, index, query;
        args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
        query = table;
        index = 0;
        parsed.where.forEach(function(x) {
          var condition;
          condition = {};
          condition[x] = args[index++];
          return query = query.where(condition);
        });
        if (parsed.withConnection) {
          connection = args[index];
          if (connection == null) {
            throw new Error(name + " must be called with connection as argument number " + (index + 1));
          }
          query = query.setConnection(connection);
        }
        return query["delete"]();
      };
    };
    factory.__inject = [config.tableToKey(parsed.table)];
    factory.__source = 'generated by fragments-postgres.delete';
    factory.__parsed = parsed;
    return factory;
  };
  return sources;
};

module.exports = configure();

module.exports.configure = configure;
