...
optionsToConfig = (options) ->
  ...
  config.nameToInsertableColumns = options.nameToInsertableColumns or (name) ->
    name + 'InsertableColumns'

  config.nameToUpdateableColumns = options.nameToUpdateableColumns or (name) ->
...
# insert
  ...
  sources.insert = (name) ->
    ...
    factory.__inject = [
      config.nameToTable(parsed.name)
      config.nameToInsertableColumns(parsed.name)
    ]
...
# update
  ...
  sources.update = (name) ->
    ...
    factory.__inject = [
      config.nameToTable(parsed.name)
      config.nameToUpdateableColumns(parsed.name)
    ]
